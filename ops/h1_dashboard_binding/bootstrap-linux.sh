...
fail() {
  printf 'H1_DASHBOARD_BOOTSTRAP_REFUSED:%s\n' "$1" >&2
  exit 4
}

if (($# != 2)); then
  fail 'usage: bootstrap-linux.sh SOURCE_ROOT SOURCE_COMMIT'
fi

SOURCE_ROOT=$1
SOURCE_COMMIT=$2
case "$SOURCE_ROOT" in
  /mnt/HC_Volume_106716684/hyperlab-h1/dashboard-sources/*-dashboard-v1) ;;
  *) fail 'source root leaves the dedicated dashboard-sources namespace' ;;
esac
...
printf 'H1_DASHBOARD_BOOTSTRAP_GREEN:%s\n' "$VENV_PYTHON"
